...
import { Mail, Phone, Eye, EyeOff, Loader2, UserPlus } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handlePhoneSignup = async (e: React.FormEvent) => {
    ...
    if (!phoneName || !phone || phone.length < 6) {
      toast.error('Please fill in all fields');
      return;
    }
    setLoading(true);
    ...
    router.push(
      `/verify-otp?phone=${encodeURIComponent(fullPhone)}&name=${encodeURIComponent(phoneName)}&type=user`
    );
  };
    ...
    <Card className="w-full max-w-md bg-[#161626] border-white/10 text-white z-10">
      <CardHeader className="text-center space-y-2">
        <div className="mx-auto p-3 rounded-xl bg-[#FF5722]/10 w-fit">
          <UserPlus className="w-6 h-6 text-[#FF5722]" />
        </div>
        <CardTitle className="text-2xl font-syne font-bold">Create Account</CardTitle>
        <CardDescription className="text-gray-400">
          Join GymSpot and start discovering gyms
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-5">
        ...
        <Tabs defaultValue="email" className="w-full">
          ...
          <TabsContent value="phone">
            <form onSubmit={handlePhoneSignup} className="space-y-4 pt-2">
